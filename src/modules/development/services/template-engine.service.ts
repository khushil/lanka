...
export class TemplateEngineService {
  ...
  async loadTemplate(criteria: {
    ...
    logger.info('Template loaded', { templateKey, templateName: template.name });
    return template;
  }

  async renderTemplate(template: CodeTemplate, data: any): Promise<string> {
    try {
      logger.info('Rendering template', { templateName: template.name });
      ...
      logger.error('Template rendering failed', { templateName: template.name, error });
      throw error;
    }
  }

  async validateTemplate(template: CodeTemplate): Promise<{
    isValid: boolean;
    errors: string[];
    warnings: string[];
  }> {
  ...
  createTemplate(template: Omit<CodeTemplate, 'id' | 'createdAt'>): CodeTemplate {
    ...
    logger.info('Template created', { templateId: newTemplate.id, templateName: newTemplate.name });
...
{{/each}}`,
  ...
  private addJavaServiceTemplate(): void {
...
{{/each}}
  ...
  private addGoServiceTemplate(): void {
...
{{/each}}
  ...
  private generateTemplateKey(criteria: {
    type: CodeTemplateType;
    language: ProgrammingLanguage;
    framework?: string;
  }): string {
  ...
  private getVariableValue(variable: TemplateVariable, data: any): string {
  ...
  private processCondition(content: string, condition: any, data: any): string {
  ...
  private processIterations(content: string, data: any): string {
  ...
  private isValidCondition(expression: string): boolean {
    // Basic validation - in production, use proper expression validation
    return /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(expression);
  }

  private formatCode(code: string, language: ProgrammingLanguage): string {
