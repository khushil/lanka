...
exports.TestPrioritizerService = void 0;
const logger_1 = require("../../../core/logging/logger");
const development_types_1 = require("../types/development.types");
class TestPrioritizerService {
    neo4j;
    constructor(neo4j) {
        this.neo4j = neo4j;
    }
    async prioritizeTests(tests, weights = {}) {
        try {
            ...
            const prioritizations = await Promise.all(enrichedTests.map(test => this.calculateTestPriority(test, finalWeights)));
    ...
    async prioritizeTestsByChanges(tests, changedFiles) {
        try {
            logger_1.logger.info('Prioritizing tests by code changes');
        ...
        `;
                ...
                const coveredFiles = result.map((r) => r.filePath);
                ...
                prioritizations.push({
                    ...
                    riskLevel: changeImpact > 0.7 ? development_types_1.Priority.HIGH :
                        changeImpact > 0.3 ? development_types_1.Priority.MEDIUM : development_types_1.Priority.LOW
                });
            }
            return prioritizations.sort((a, b) => b.priority - a.priority);
        }
        catch (error) {
            logger_1.logger.error('Failed to prioritize tests by changes', { error: error.message });
            throw new Error('Failed to prioritize tests by changes');
        }
    }
    async prioritizeTestsWithBudget(tests, timeBudgetMinutes, weights = {}) {
        try {
            logger_1.logger.info('Prioritizing tests with time budget', { budget: timeBudgetMinutes });
            ...
            logger_1.logger.info('Selected tests within budget', {
            ...
            logger_1.logger.error('Failed to prioritize tests with budget', { error: error.message });
    ...
    async calculateRiskScore(testData) {
        try {
            ...
            if (testData.avgExecutionTime > 5000)
                riskScore += 2;
            else if (testData.avgExecutionTime > 2000)
                riskScore += 1;
            // Complexity risk (0-2 points)
            if (testData.complexity > 8)
                riskScore += 2;
            else if (testData.complexity > 5)
                riskScore += 1;
            // Dependencies risk (0-2 points)
            const externalDeps = testData.dependencies.filter(dep => dep.includes('api') || dep.includes('database') || dep.includes('external'));
            if (externalDeps.length > 2)
                riskScore += 2;
            ...
            logger_1.logger.error('Failed to calculate risk score', { error: error.message });
            return 5; // Default medium risk
        }
    }
    async analyzeTestDependencies(testIds) {
        try {
            logger_1.logger.info('Analyzing test dependencies');
        ...
        `;
                ...
                const dependencies = result.map((r) => r);
                ...
                dependencyAnalysis[testId] = {
                    ...
                    dependencyRisk,
                    issues,
                    externalDependencies: dependencies.filter((dep) => dep.type === 'EXTERNAL_API').length,
                    databaseDependencies: dependencies.filter((dep) => dep.type === 'DATABASE').length
                };
            }
            ...
            logger_1.logger.error('Failed to analyze test dependencies', { error: error.message });
            throw new Error('Failed to analyze test dependencies');
        }
    }
    async optimizeTestOrder(tests, options = {}) {
        try {
            logger_1.logger.info('Optimizing test execution order');
            ...
            logger_1.logger.error('Failed to optimize test order', { error: error.message });
            return tests; // Return original order if optimization fails
        }
    }
    async analyzeFlakiness(testIds) {
        try {
            logger_1.logger.info('Analyzing test flakiness');
        ...
        `;
                ...
                flakinessAnalysis[testId] = {
                    ...
                    patterns,
                    recommendation,
                    recentFailures: runs.filter((run) => !run.passed).length,
            ...
            logger_1.logger.error('Failed to analyze flakiness', { error: error.message });
            throw new Error('Failed to analyze flakiness');
        }
    }
    async prioritizeByRegressionRisk(testIds, recentChanges) {
        try {
            logger_1.logger.info('Prioritizing by regression risk');
            const prioritizations = [];
            for (const testId of testIds) {
        ...
        `;
                ...
                const regressionRisk = this.calculateRegressionRisk(testData.filePath, recentChanges, testData.regressionHistory);
                const factors = [
                    {
                ...
                prioritizations.push({
                    ...
                    riskLevel: regressionRisk.totalRisk > 0.7 ? development_types_1.Priority.CRITICAL :
                        regressionRisk.totalRisk > 0.4 ? development_types_1.Priority.HIGH :
                            regressionRisk.totalRisk > 0.2 ? development_types_1.Priority.MEDIUM : development_types_1.Priority.LOW
                });
            }
            return prioritizations.sort((a, b) => b.priority - a.priority);
        }
        catch (error) {
            logger_1.logger.error('Failed to prioritize by regression risk', { error: error.message });
            throw new Error('Failed to prioritize by regression risk');
        }
    }
    async adjustPrioritiesWithFeedback(initialPriorities, feedback) {
        try {
            logger_1.logger.info('Adjusting priorities with feedback');
            return initialPriorities.map(priority => {
                ...
                if (testFeedback.actualDuration > 2000)
                    adjustment -= 1;
                if (testFeedback.actualDuration < 100)
                    adjustment += 0.5;
                // Adjust based on pass/fail
                if (!testFeedback.passed)
                    adjustment -= 2;
                if (testFeedback.passed && testFeedback.issues.length === 0)
                    adjustment += 0.5;
                // Adjust based on issues
                if (testFeedback.issues.includes('flaky'))
                    adjustment -= 1.5;
                if (testFeedback.issues.includes('timeout'))
                    adjustment -= 1;
            ...
            logger_1.logger.error('Failed to adjust priorities with feedback', { error: error.message });
            return initialPriorities;
        }
    }
    async extractLearningsFromHistory(executionHistory) {
        try {
            logger_1.logger.info('Extracting learnings from execution history');
            const learnings = {};
            // Group by test ID
            ...
            logger_1.logger.error('Failed to extract learnings from history', { error: error.message });
            throw new Error('Failed to extract learnings from history');
        }
    }
    async optimizeForResources(tests, resources) {
        try {
            logger_1.logger.info('Optimizing for available resources');
            const prioritizations = [];
            for (const test of tests) {
                ...
                prioritizations.push({
                    ...
                    riskLevel: resourceScore > 0.8 ? development_types_1.Priority.LOW :
                        resourceScore > 0.5 ? development_types_1.Priority.MEDIUM : development_types_1.Priority.HIGH
                });
            }
            return prioritizations.sort((a, b) => b.priority - a.priority);
        }
        catch (error) {
            logger_1.logger.error('Failed to optimize for resources', { error: error.message });
            throw new Error('Failed to optimize for resources');
        }
    }
    async prioritizeForPipeline(tests, pipelineContext) {
        try {
            logger_1.logger.info('Prioritizing for CI/CD pipeline');
            ...
            if (pipelineContext.timeConstraint) {
                const budgetTests = await this.prioritizeTestsWithBudget(prioritizedTests, pipelineContext.timeConstraint / 60);
            ...
            logger_1.logger.error('Failed to prioritize for pipeline', { error: error.message });
            return tests;
        }
    }
    async updatePrioritiesFromResults(testResults) {
        try {
            logger_1.logger.info('Updating priorities from test results');
            const updates = {};
            ...
            logger_1.logger.error('Failed to update priorities from results', { error: error.message });
            throw new Error('Failed to update priorities from results');
        }
    }
    // Private helper methods
    async enrichTestsWithData(tests) {
        const enrichedTests = [];
        for (const test of tests) {
    ...
    async calculateTestPriority(test, weights) {
        const factors = [];
    ...
    calculateChangeImpact(coveredFiles, changedFiles) {
        if (coveredFiles.length === 0)
            return 0;
        const intersection = coveredFiles.filter(file => changedFiles.some(changed => changed.includes(file) || file.includes(changed)));
        return intersection.length / coveredFiles.length;
    }
    calculateDependencyRisk(dependencies) {
        let risk = 0;
        dependencies.forEach(dep => {
            const criticality = dep.criticality || 5;
            if (dep.type === 'EXTERNAL_API')
                risk += criticality * 0.3;
            if (dep.type === 'DATABASE')
                risk += criticality * 0.2;
            if (dep.type === 'SERVICE')
                risk += criticality * 0.15;
        });
    ...
    identifyDependencyIssues(dependencies) {
        ...
        if (dependencyNames.length !== new Set(dependencyNames).size) {
            issues.push({ type: 'DUPLICATE_DEPENDENCY', severity: development_types_1.Priority.MEDIUM });
        }
        ...
        if (externalDeps.length > 3) {
            issues.push({ type: 'TOO_MANY_EXTERNAL_DEPS', severity: development_types_1.Priority.HIGH });
        }
        return issues;
    }
    detectCircularDependencies(dependencyAnalysis) {
        ...
        for (const [testId, analysis] of Object.entries(dependencyAnalysis)) {
            const circularDeps = analysis.dependencies.filter((dep) => dep.name === testId || dep.name.includes(testId));
            if (circularDeps.length > 0) {
                analysis.issues.push({ type: 'CIRCULAR_DEPENDENCY', severity: development_types_1.Priority.HIGH });
            }
        }
    }
    groupTestsBySetup(tests) {
    ...
    orderGroupsForEfficiency(groups) {
    ...
    orderByExecutionTime(tests, allowParallel) {
        if (allowParallel) {
    ...
    calculateFlakinessScore(runs) {
        ...
        for (let i = 0; i <= runs.length - windowSize; i++) {
            ...
            const passCount = window.filter((run) => run.passed).length;
    ...
    identifyFlakinessPatterns(runs) {
        const patterns = [];
        // Environment-based pattern
        const environments = runs.reduce((env, run) => {
            if (!env[run.environment])
                env[run.environment] = { passed: 0, failed: 0 };
            if (run.passed)
                env[run.environment].passed++;
            else
                env[run.environment].failed++;
            return env;
        }, {});
    ...
    generateFlakinessRecommendation(score, patterns) {
        if (score > 0.5) {
    ...
    calculateRegressionRisk(filePath, recentChanges, regressionHistory) {
        // Change impact
        const relevantChanges = recentChanges.filter(change => filePath.some(file => change.file.includes(file)));
    ...
    generateLearningRecommendations(patterns, trends) {
    ...
    calculateResourceCompatibility(usage, available) {
        if (!usage || !available)
            return 0.5;
        let compatibility = 0;
        let factorCount = 0;
    ...
    async filterTestsByRelevance(tests, changedFiles) {
        // Simplified relevance filtering
        return tests.filter(test => test.tags.some(tag => changedFiles.some(file => file.includes(tag))) ||
            test.requirements.length > 0);
    }
    prioritizeByQualityGates(tests, qualityGates) {
    ...
    generateReasoning(factors) {
        const topFactor = factors.reduce((max, factor) => factor.contribution > max.contribution ? factor : max);
        return `Primary factor: ${topFactor.name} (${Math.round(topFactor.contribution * 100)}% contribution)`;
    }
    determineRiskLevel(priority) {
        if (priority >= 8)
...
exports.TestPrioritizerService = TestPrioritizerService;
//# sourceMappingURL=test-prioritizer.service.js.map